...
namespace ViewModel
{
    public class ConstructorViewModel : MonoBehaviour
    {
        ...
        [Inject] private readonly ISharingManager _sharing;
        ...
        public int GetDefaultKey(ItemId<Component> componentId)
        {
            ...
            var items = ShipLayoutViewModel.Components.Concat(LeftPlatformLayoutViewModel.Components)
        ...
        public void OnComponentInstalled(ComponentInfo component)
        {
            ...
            var shouldShowComponent =
                ComponentInfo.gameObject.activeSelf && _components.Items.GetQuantity(component) > 0;
        ...
        private BitArray SerializeShip()
        {
            SaveCurrentShip();
            var data = Ship.ToShipData().Serialize().ToArray();
        ...
        private IEnumerator TakeScreenshotAndShare()
        {
            ...
            var ss = ScreenshotUtils.TakeScreenshot();

            ScreenshotUtils.MakeTextureFullyOpaque(ss);

            var bits = SerializeShip();

            bits.WriteIntoAlpha(ss);

            ss.Apply();

            _sharing.ShareScreenshot(ss, $"{Ship.Name}_{DateTimeOffset.Now.ToUnixTimeMilliseconds()}.png", "builds");
        ...
        private void Initialize(IShip ship)
        {
            ...
            _leftPlatformLayout = ship.FirstSatellite != null
                ? new ShipLayout(ship.FirstSatellite.Information.Layout, ship.FirstSatellite.Information.Barrels,
                    ship.FirstSatellite.Components, debug)
                : null;
            _rightPlatformLayout = ship.SecondSatellite != null
                ? new ShipLayout(ship.SecondSatellite.Information.Layout, ship.SecondSatellite.Information.Barrels,
                    ship.SecondSatellite.Components, debug)
                : null;
        ...
        private void ResetLayout()
        {
            ...
            if (_leftPlatformLayout != null)
            {
                ...
                LeftPlatformLayoutViewModel.BackgroundImage.sprite =
                    _resourceLocator.GetSprite(Ship.FirstSatellite.Information.ModelImage);
            }
            else
            {
            ...
            if (_rightPlatformLayout != null)
            {
                ...
                RightPlatformLayoutViewModel.BackgroundImage.sprite =
                    _resourceLocator.GetSprite(Ship.SecondSatellite.Information.ModelImage);
            }
            else
            {
        ...
        private bool HasUnlockedComponents
        {
            get
            {
                ...
                if (_leftPlatformLayout != null && _leftPlatformLayout.Components.Any(item => !item.Locked))
                    return true;
                if (_rightPlatformLayout != null && _rightPlatformLayout.Components.Any(item => !item.Locked))
                    return true;
                return false;
            }
        }
        ...
        private void UpdateStats()
        {
            ...
            if (_leftPlatformLayout != null)
                builder.AddSatellite(
                    new CommonSatellite(Ship.FirstSatellite.Information, _leftPlatformLayout.Components),
                    CompanionLocation.Left);
            if (_rightPlatformLayout != null)
                builder.AddSatellite(
                    new CommonSatellite(Ship.SecondSatellite.Information, _rightPlatformLayout.Components),
                    CompanionLocation.Right);
